...
//+++ BRCM_LOCAL
		...
		debug("discover_services: remove attrId 0x004 if available");
        /* get record based on UUID pattern  */
        rec = btd_device_get_record(device, pattern);
        if(rec)
        {
            debug("***** discover_services: got record %p", rec);
            /* remove SCN ATTR from record      should be 0x0004  */
            sdp_attr_remove(rec, 0x0004);
        }
        else
            debug("***** discover_services: no rec available to cleanup *****");
//--- BRCM_LOCAL
...
//+++ BRCM_LOCAL CASE ID 329812  not match up with hcidefs.h 0x06->0x0F
	case 0x0F: /* AUTHENTICATION_REJECTED */
	/* Google Original
	case 0x06:
	*/
//--- BRCM  CASE ID 329812 end
