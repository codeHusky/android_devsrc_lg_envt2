...
// START_LGE, hopemini@lge.com, 2009-12-08, QCT
#if 1//def LG_AUDIO_BROADCOM_BT
...
#endif
// END_LGE, hopemini@lge.com, 2009-12-08, QCT
...
unsigned char SendCommand(int fd, unsigned short opcode, unsigned char param_len, unsigned char *p_param_buf)
{
    unsigned char  pbuf[255] = {0,};
    unsigned char  i=0;
    
    pbuf[0] = 0x1;
    pbuf[1] = (unsigned char)(opcode);
    pbuf[2] = (unsigned char)(opcode >>8);
    pbuf[3] = param_len;
...
static int bcm4325(int fd, struct uart_t *u, struct termios *ti)
{
    ...
    switch(baudrate)
    {
        ...
        case 2500000:
            ...
            SendCommand(fd, VSC_WRITE_UART_CLOCK_SETTING, VSC_WRITE_UART_CLOCK_SETTING_LEN, (unsigned char *)&uart_clock_24);
        ...
        case 4000000:
            ...
            SendCommand( fd, VSC_WRITE_UART_CLOCK_SETTING, VSC_WRITE_UART_CLOCK_SETTING_LEN, (unsigned char *)&uart_clock_48);
            read_event(fd, buffer);        
            hci_data[2] = baudrate & 0xFF;
            hci_data[3] = (baudrate >> 8) & 0xFF;
            hci_data[4] = (baudrate >> 16) & 0xFF;
        ...
        default:
            //LOGE("Not Support baudrate = %d", baudrate);
            return -1;
            break;
    }

    //LOGE("SEND BAUDRATE COMMAND\n");
    SendCommand(fd,  HCI_BRCM_UPDATE_BAUDRATE_CMD, HCI_BRCM_UPDATE_BAUD_RATE_UNENCODED_LENGTH, (unsigned char *)hci_data);
    read_event(fd, buffer);

    //LOGE("Setting Phone Baudrate\n");
    ...
    tcflush(fd, TCIOFLUSH);
    tcflush(fd, TCIOFLUSH);
    cfsetospeed(ti, uart_speed(baudrate));
    cfsetispeed(ti, uart_speed(baudrate));
    tcsetattr(fd, TCSANOW, ti);
...
#if 1//def LG_AUDIO_BROADCOM_BT
	/* Broadcom BCM4325 */
	{ "bcm4325",	0x0000, 0x0000, HCI_UART_H4,   921600, 921600, FLOW_CTL, NULL, bcm4325	},	
#endif
// END_LGE, hopemini@lge.com, 2009-12-08, QCT
...
// START_LGE, hopemini@lge.com, 2009-12-08, QCT
#if 1//def LG_AUDIO_BROADCOM_BT
/* Finalize UART driver - added by obd. for patch ram download*/
int finalize_uart(char *dev)
{
	struct termios ti;
	int fd;

	//LOGE("Finalize hciattach uart %s : dev\n", dev);
	...
	/* Set actual baudrate - default UART speed is 115200*/
