...
static void auto_rename(DOS_FILE *file)
{
	...
	number++;
/* LGE_CHANGE_S : checking retries will be until 99. (org = 9999999) */
	if (number > 99) {
...
static int handle_dot(DOS_FS *fs,DOS_FILE *file,int dots)
{
	...
	switch (interactive ? get_key("1234","?") : '2') {
		...
		return 1;
	    case '2':
/* LGE_CHANGE_S : Skip this part for not changing the file name */
/* original
		auto_rename(file);
...
/* LGE_CHANGE_S : Skip bad file name in order not to make watchdog in case of crashed dentry */
...
/* LGE_COMMENT : if duplcated names are not handled, futher access or write operation possibly make another dentry crash */
