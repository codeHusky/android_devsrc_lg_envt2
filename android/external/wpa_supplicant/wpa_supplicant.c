...
int prev_auth_timeout_cnt = 0;
unsigned char prev_auth_timeout_ssid[ETH_ALEN] = {0, };
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-09-10, < Protect the deauthentication DoS from same AP */
...
static void wpa_supplicant_scan_broadcast(void *eloop_ctx, void *timeout_ctx);
int wpa_inital_scan_onoging = 0;
#endif
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_s->reassociate = 1;
/* LGE_CHANGE_S, [yoohoo@lge.com], 2009-9-18, Mac filtering issue */
#ifdef WPA_SUPPLICANT_PATCH	
/* If there is one config profile and that profile is a black list(count2) give 20 second delay before sending scan command */
	if ( os_memcmp(prev_auth_timeout_ssid, bssid, ETH_ALEN) == 0) {
		prev_auth_timeout_cnt++;
		wpa_printf(MSG_DEBUG, "wpa_supplicant_timeout : Increase prev_auth_timeout_cnt %d", prev_auth_timeout_cnt);
	} else {
		prev_auth_timeout_cnt=0;
		os_memcpy(prev_auth_timeout_ssid, bssid, ETH_ALEN);
		wpa_printf(MSG_DEBUG, "wpa_supplicant_timeout : Initialise prev_auth_timeout_cnt %d", prev_auth_timeout_cnt);
	}

	if ( prev_auth_timeout_cnt > 1 ) {
		prev_auth_timeout_cnt = 0;
		wpa_printf(MSG_DEBUG, "wpa_supplicant_timeout : Delayed SCAN");
		wpa_supplicant_req_scan(wpa_s, 20, 0);
	} else
		wpa_supplicant_req_scan(wpa_s, 0, 0);
#else
 	wpa_supplicant_req_scan(wpa_s, 0, 0);
#endif
/* LGE_CHANGE_E, [yoohoo@lge.com], 2009-9-18,  Mac filtering issue */
}
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	while (ssid) {
		if (!ssid->disabled &&
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-07-06, <Do broadcast scan even thouhg ap_scan==2> */
#ifdef WPA_SUPPLICANT_PATCH			
		    (ssid->scan_ssid))
#else
...
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-07-04, <Do broadcast scan after specific> */
#ifdef WPA_SUPPLICANT_PATCH
//		eloop_register_timeout(1, 0, wpa_supplicant_scan_broadcast, wpa_s, NULL);
#endif
/* LGE_CHANGE_E, [jisung.yang@lge.com], 2010-07-04, <Do broadcast scan after specific> */
...
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-07-04, <Do broadcast scan after specific> */
#ifdef WPA_SUPPLICANT_PATCH
//	eloop_register_timeout(0, 0, wpa_supplicant_scan_broadcast, wpa_s, NULL);
	wpa_printf(MSG_INFO, "Try to do Initial scan(broadcast)");
	wpa_drv_scan(wpa_s, NULL, 0);
	wpa_s->scan_ongoing = 1;
	wpa_inital_scan_onoging = 1;
...
static void wpa_supplicant_scan_broadcast(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->use_client_mlme) {
		ret = ieee80211_sta_req_scan(wpa_s, NULL, 0);
	} else {
		ret = wpa_drv_scan(wpa_s, NULL, 0);
	}
	if (ret) {
		wpa_printf(MSG_WARNING, "Failed to initiate AP scan.");
	}
