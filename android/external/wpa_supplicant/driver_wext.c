...
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-10-06, BCM4325 update wl status after receiving disassoc */
#ifdef WPA_SUPPLICANT_PATCH
static int wpa_driver_wext_disassociate(void *priv, const u8 *addr,
					int reason_code);
#endif
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-10-06, BCM4325 update wl status after receiving disassoc */
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case SIOCGIWAP:
			...
			if (os_memcmp(iwe->u.ap_addr.sa_data,
				...
				os_free(drv->assoc_resp_ies);
				drv->assoc_resp_ies = NULL;
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-10-24, BCM4325 update wl status after receiving disassoc. */
/* If not updating wl status, bcm4325 current consumption is about 70mA. */
#ifdef WPA_SUPPLICANT_PATCH
				struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)ctx;
				if (os_memcmp(wpa_s->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN) != 0) { /* bssid is valid */
					wpa_driver_wext_disassociate(drv, (unsigned char *)"\x00\x00\x00\x00\x00\x00", REASON_DEAUTH_LEAVING);
					wpa_printf(MSG_DEBUG, "Do disassociation explicitly from wpa_supplicant");
				}
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-10-24, BCM4325 update wl status after receiving disassoc */
...
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-10-24, for bcm4325 */
#ifdef WPA_SUPPLICANT_PATCH
				/* Implement a range for dBm [-192; 63] */
				if (iwe->u.qual.noise >= 64)
					results[ap_num].noise -= 0x100;
				if (iwe->u.qual.level >= 64)
					results[ap_num].level -= 0x100;
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-10-24, for bcm4325 */
			}
			break;
		case SIOCGIWENCODE:
			if (ap_num < max_size &&
...
static char *wpa_driver_get_country_code(int channels)
{
	...
	char *country="XW"; //yangseon.so 2010.11.15
...
/* LGE_CHANGE_S [jisung.yang@lge.com],  2010-07-05, <Enable set channel in Android> */	
#ifndef WPA_SUPPLICANT_PATCH
...
#endif	
/* LGE_CHANGE_E [jisung.yang@lge.com],  2010-07-05, <Enable set channel in Android> */	
...
/* LGE_CHANGE_S [jisung.yang@lge.com],  2010-05-03, Do not count in case of RSSI */		
		// rssi is considered only at the above of associated
#ifdef WPA_SUPPLICANT_PATCH		
		if((os_strcmp(cmd, "RSSI") == 0 || (os_strcmp(cmd, "LINKSPEED") == 0 )) && wpa_s->wpa_state < WPA_ASSOCIATED)
			return ret;		
#endif		
/* LGE_CHANGE_E [jisung.yang@lge.com],  2010-05-03, Do not count in case of RSSI */		
...
/* LGE_CHANGE_S [jisung.yang@lge.com],  2010-07-05, <LGE add channel setting and calling statistics> */		
#ifdef WPA_SUPPLICANT_PATCH		
		    (os_strcasecmp(cmd, "DEV-STATS") == 0) ||
