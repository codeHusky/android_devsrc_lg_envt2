...
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-09-10, < Protect the deauthentication DoS from same AP */
#ifdef WPA_SUPPLICANT_PATCH
unsigned char g_prev_connected_ap[ETH_ALEN] = {0, };
unsigned char g_last_connected_ap[ETH_ALEN] = {0, };
struct os_time prev_last_deauth_tick = {0, 0};
extern int prev_auth_timeout_cnt;
extern unsigned char prev_auth_timeout_ssid[ETH_ALEN];
#define SUCCESSIVE_DEAUTH_INTERVAL 5
extern int scan_progress_flag;
extern wpa_states wpa_supplicant_get_state(struct wpa_supplicant *wpa_s);
extern int wpa_inital_scan_onoging; // jisung. for test
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-08-05, < Protect the deauthentication DoS from same AP */
...
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-07-04, <Do broadcast scan at the first time */
#ifdef WPA_SUPPLICANT_PATCH
	if (wpa_inital_scan_onoging == 1 ){
		wpa_inital_scan_onoging = 0;
		wpa_printf(MSG_INFO, "wpa_inital_scan_onoging was set. Discard scan result");
		return;
	}
#endif	
/* LGE_CHANGE_E, [jisung.yang@lge.com], 2010-07-04, <Do broadcast scan at the first time */
	...
	wpa_printf(MSG_INFO, "wpa_supplicant_event_scan_results: ap_scan %d,  disconnected %d ", wpa_s->conf->ap_scan, wpa_s->disconnected);
...
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-09-10, < Protect the deauthentication DoS from same AP */
#ifdef WPA_SUPPLICANT_PATCH		
		os_memcpy(g_prev_connected_ap, g_last_connected_ap, ETH_ALEN);
		os_memcpy(g_last_connected_ap, bssid, ETH_ALEN);
		prev_auth_timeout_cnt = 0;
		os_memset(prev_auth_timeout_ssid, 0, ETH_ALEN);
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-08-05, < Protect the deauthentication DoS from same AP */		
	...
	/* LGE_CHANGE_S, [youngsin.lee@lge.com] */
	// set eapol_start timeout to 3 seconds
	eapol_sm_configure(wpa_s->eapol, -1, -1, 3, -1);	
	// have eapol_port_timer newly registered for the timer is not there 
	eapol_sm_port_timer_trigger(wpa_s->eapol);
...
static void wpa_supplicant_event_disassoc(struct wpa_supplicant *wpa_s)
{
	const u8 *bssid;
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-09-10, < Protect the deauthentication DoS from same AP */
/* Protect successive deauth in SUCCESSIVE_DEAUTH_INTERVAL secs */
#ifdef WPA_SUPPLICANT_PATCH
	struct os_time tv;
	struct os_time now;
	unsigned char delay_scan = 0;
#define DELAY_SCAN_SECS 20

	os_get_time(&now);
	os_time_sub(&now, &prev_last_deauth_tick, &tv);
	if(os_memcmp(wpa_s->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN) != 0 
		&& tv.sec >= 0 && tv.sec <= SUCCESSIVE_DEAUTH_INTERVAL) {
		if (os_memcmp(g_last_connected_ap, "\x00\x00\x00\x00\x00\x00", ETH_ALEN) != 0
			&& os_memcmp(g_last_connected_ap, g_prev_connected_ap, ETH_ALEN) == 0) {
#if 0			
			delay_scan = 1;
			wpa_printf(MSG_DEBUG, "SUCCESSIVE DEAUTH in %d secs from BSSID" MACSTR, 
				SUCCESSIVE_DEAUTH_INTERVAL, MAC2STR(g_last_connected_ap));
#else	// Add to blacklist and give delay
			const u8 *bssid = wpa_s->bssid;
			wpa_blacklist_add(wpa_s, bssid);
			delay_scan = 1;
			wpa_printf(MSG_INFO, "Add to Blacklist");
#endif
		}
	}
	prev_last_deauth_tick.sec = now.sec;
	prev_last_deauth_tick.usec = now.usec;
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-08-05, < Protect the deauthentication DoS from same AP */
...
/* LGE_CHANGE_S [yoohoo@lge.com] 2009-09-10, < Protect the deauthentication DoS from same AP */
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) { 
		if(delay_scan)
			wpa_supplicant_req_scan(wpa_s, DELAY_SCAN_SECS, 0);
		else
			wpa_supplicant_req_scan(wpa_s, 0, 100000);
	}
#else /* WPA_SUPPLICANT_PATCH */
	if (wpa_s->wpa_state >= WPA_ASSOCIATED)
		wpa_supplicant_req_scan(wpa_s, 0, 100000);
#endif /* WPA_SUPPLICANT_PATCH */
/* LGE_CHANGE_E [yoohoo@lge.com] 2009-08-05, < Protect the deauthentication DoS from same AP */	
