...
#endif
#endif
#ifdef ANDY_ERS //ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] ERS porting
#include <libers/android_ers_reporter.h>
#include <libers/ers_utils.h>
#endif
                         ...
                         bool at_fault)
{
    ...
    while (p <= end) {
         ...
         p += 4;
        #ifdef ANDY_ERS //ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] ERS porting
		 store_stack_frame(p,data,map_to_name(map, data, ""));
        #endif
    }
    ...
    while (p <= end) {
        ...
        #ifdef ANDY_ERS
        //ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] ERS porting
		 store_stack_frame(p,data,map_to_name(map, data, ""));
        #endif
                    ...
                    bool at_fault)
{
	...
	//ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] ERS porting
    #ifdef ANDY_ERS
    store_pc_lr_backtrace("pc",r.ARM_pc,map_to_name(map, r.ARM_pc, "<unknown>"));
    #endif
    }
    _LOG(tfd, !at_fault, "         #%02d  lr %08x  %s\n", 1, r.ARM_lr,
            map_to_name(map, r.ARM_lr, "<unknown>"));
    #ifdef ANDY_ERS
    store_pc_lr_backtrace("lr",r.ARM_lr,map_to_name(map, r.ARM_lr, "<unknown>"));
    #endif
...
void dump_fault_addr(int tfd, int pid, int sig)
{
    ...
    #ifdef ANDY_ERS //ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] fix bug that don't trace pid in android ers repor
		store_fault_addr((unsigned long)si.si_addr);
    #endif
...
static void handle_crashing_process(int fd)
{
    ...
    for(;;) {
        ...
        if(WIFSTOPPED(status)){
            ...
            #ifdef ANDY_ERS //ANDY_PORTING ERS [wonkyoung.cho@lge.com 100401] add ERS enabler functions
				/* get the status enabled ERS on/off */
				android_ers_getStatus();
				android_ers_log_dump();
                need_cleanup = engrave_tombstone(cr.pid, tid, debug_uid, n);
				report_android_ers(cr.pid,tid,n);
                kill(tid, SIGSTOP);
				/* just one time take crashing the information */
				/*whether if ers on, reboot or not */
				android_ers_reboot();
			#else
                need_cleanup = engrave_tombstone(cr.pid, tid, debug_uid, n);
                kill(tid, SIGSTOP);			
